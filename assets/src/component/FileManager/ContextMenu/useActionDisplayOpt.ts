...
export const getActionOpt = (
  ...
  if (type == ContextMenuTypes.empty || type == ContextMenuTypes.new) {
    ...
    // In the group share area members do not own the folders, but the backend
    // authorises them via navigator capabilities, so don't gate on ownership there.
    const parentIsGroupShare = parent.path ? new CrUri(parent.path).fs() == Filesystem.group : false;
    display.showCreateFolder = parentCap.enabled(NavigatorCapability.create_file) && (parent.owned || parentIsGroupShare);
  ...
  const inGroupShare = parentUrl.fs() == Filesystem.group;
  targets.forEach((target) => {
    let readable = true;
    // Group share members can modify files they don't own (authorised by capabilities).
    let updatable = (target.owned || inGroupShare) && parentUrl.fs() != Filesystem.share;

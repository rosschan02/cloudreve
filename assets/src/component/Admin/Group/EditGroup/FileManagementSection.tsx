...
import { GroupPermission, User } from "../../../../api/user";
import Boolset from "../../../../util/boolset";
import UserSearchInput from "../../File/UserSearchInput";
...
const FileManagementSection = () => {
  ...
  const { values, setGroup, setShareRootOwnerHash } = useContext(GroupSettingContext);
  ...
  const onAllowGroupShareChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      setGroup((p: GroupEnt) => ({
        ...p,
        permissions: new Boolset(p.permissions).set(GroupPermission.access_group_share, e.target.checked).toString(),
      }));
    },
    [setGroup],
  );

  const onShareOwnerSelected = useCallback(
    (user: User) => {
      setShareRootOwnerHash?.(user.id);
      setSelectedOwnerName(`${user.nickname}${user.email ? ` <${user.email}>` : ""}`);
    },
            ...
            <SettingForm lgWidth={5}>
              <FormControl fullWidth>
                <FormControlLabel
                  control={
                    <Switch
                      checked={permission.enabled(GroupPermission.access_group_share)}
                      onChange={onAllowGroupShareChange}
                    />
                  }
                  label={t("group.accessGroupShare")}
                />
                <NoMarginHelperText>{t("group.accessGroupShareDes")}</NoMarginHelperText>
              </FormControl>
            </SettingForm>
            <Collapse in={permission.enabled(GroupPermission.access_group_share)} unmountOnExit>
              <SettingForm title={t("group.groupShareOwner")} lgWidth={5}>
                <FormControl fullWidth>
                  <UserSearchInput onUserSelected={onShareOwnerSelected} label={t("group.groupShareOwnerSelect")} />
                  <NoMarginHelperText>
                    {selectedOwnerName
                      ? t("group.groupShareOwnerSelected", { user: selectedOwnerName })
                      : values.settings?.share_root_owner
                        ? t("group.groupShareOwnerCurrent", { id: values.settings.share_root_owner })
                        : t("group.groupShareOwnerDes")}
                  </NoMarginHelperText>
                </FormControl>
              </SettingForm>
            </Collapse>
